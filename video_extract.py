...
for i in range(len(mots)):
    ...
    if i < len(mots) - 1:
        avgMots[i] += mots[i+1]
        divide += 1
    avgMots[i] = (np.array(avgMots[i], dtype='f')/3).tolist()
    if pmfSpeed == None:
        pmfSpeed = pmf.Pmf(avgMots[i][0])
        for j in range(1, len(avgMots[i])):
            pmfSpeed.__add__(pmf.Pmf(avgMots[i][j]))
    else:
        for speedArray in avgMots[i]:
            pmfSpeed.__add__(pmf.Pmf(speedArray))
